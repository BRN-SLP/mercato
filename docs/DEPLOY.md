...
Fund the deployer with a tiny amount of testnet CELO from the Celo Sepolia faucet plus at least 50 cUSD (used to seed the reward pool), then:
...
Now seed the reward pool (50 cUSD by default):

```bash
PROXY_ADDRESS=<proxy-from-above> pnpm seed:celo-sepolia
```

This transfers cUSD from the deployer into the proxy. The contract uses that balance to pay out `claimRewards()` calls. Once depleted, transfers can be topped up at any time — `cUSD.transfer(proxy, amount)` from any address is sufficient.
...
Same steps with `pnpm deploy:celo` and `pnpm seed:celo`. The cUSD mainnet address is hardcoded in the script (`KNOWN_CUSD[42220]`). The deployer key MUST be fully isolated from any personal wallet.
...
| `NEXT_PUBLIC_PRICE_ORACLE_ADDRESS_MAINNET` | proxy address from `pnpm deploy:celo` |
| `NEXT_PUBLIC_PRICE_ORACLE_ADDRESS_SEPOLIA` | proxy address from `pnpm deploy:celo-sepolia` |
...
The contract is UUPS-upgradeable. To ship a V2 with anti-Sybil restrictions (wallet-age threshold, daily submission caps, bond requirement, automatic outlier rejection), write `PriceOracleV2.sol` extending `PriceOracle`, then:
